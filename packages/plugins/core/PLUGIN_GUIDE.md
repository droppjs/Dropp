...
**Package:** `@dropp/plugin-watermark`
...
**Package:** `@dropp/plugin-ai-tagging`
...
**Package:** `@dropp/plugin-seo`

**Installation:**

```bash
dropp plugin:install seo
pnpm add @dropp/plugin-seo
```
...
for (const [name, settings] of Object.entries(dropp.plugins ?? {})) {
  if (settings.enabled) {
    const PluginClass = await import(`@dropp/plugin-${name}`);
...
1. Create package with `@dropp/plugin-*` naming
2. Implement `MediaPlugin` interface
3. Export plugin class as default
4. Add to `@dropp/plugins-core` peerDependency
